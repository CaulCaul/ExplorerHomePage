...
| 光晕：单个 rAF 循环 lerp 跟随；浅色主题 multiply / 深色 screen；大小（200–1200px）与透明度（--glow-o 变量，0.1–1，深色主题 ×1.1）设置页可调 | 曾因低透明度浅色渐变在浅背景下不可见（用户反馈），混合模式保证任何背景可见；尺寸/强度由用户调节 |
...
- **M2（进行中）**：已完成双通道角标（单气泡双行）、设置抽屉（分区：外观/光晕/图片展示/数据/关于）、浅色默认主题、拖拽排序、鼠标光晕（混合模式修复可见性，大小/透明度可调）、图片展示（模糊修复、焦点裁剪、宽度 15%–50% 可调、右缘锐利阴影+描边、开合滑动动画）、添加按钮与磁贴统一风格；剩余：候选关键词高亮、可配置探针间隔/历史上限；
