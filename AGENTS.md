...
- **M2（进行中）**：已完成双通道角标（单气泡双行）、设置抽屉（分区：外观/光晕/图片展示/数据/关于）、浅色默认主题、拖拽排序、鼠标光晕（混合模式修复可见性，大小/透明度可调，零延迟直跟）、图片展示（模糊修复、焦点裁剪、宽度 15%–50% 可调、右缘锐利阴影+描边、开合滑动动画）、添加按钮与磁贴统一风格（悬停背景等宽）、引擎选择器内嵌搜索框（图标+滑块+分割线；Bing 图标为几何 b 字形渐变 SVG）、搜索按钮内缩圆角与外框嵌套；剩余：候选关键词高亮、可配置探针间隔/历史上限；
